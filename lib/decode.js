function decode (h) {
	...
	if (hash) {
		...
		for (var o in octets) bits = bits.concat(bitwise.byte.read(octets[o]));
		bits.shift();
		...
		for (var i=0; i<bits3x15.length; i++) {
			...
			if (bits3) decoded.digits["digit"+(i+1)] = [parseInt(bits3, 2), bits3];
			previousCursor += 3;
		}
		return decoded;
	}
...
//String.prototype.h3ToBit = decode;
